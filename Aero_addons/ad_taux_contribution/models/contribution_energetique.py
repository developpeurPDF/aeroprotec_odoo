...
class RetenueGarantie(models.Model):
    _name = 'contribution.energetique'
    ...
    def action_confirm(self):
        for rec in self:
            if rec.name in [_('Contribution énergétique Number'),_('New')]:
                rec.name = self.env['ir.sequence'].next_by_code('seq.retenue.energie') or _('New')
