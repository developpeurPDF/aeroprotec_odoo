...
class MergeSameCustomerSaleOrder(models.TransientModel):
    ...
    def update_so_list(self):
        if self.partner_id:
            orders = self.env['sale.order'].sudo().search([('partner_id','=',self.partner_id.id),('state','=', 'draft')])
    ...
    def merge_same_customer_sale_orders(self):
        ...
        sales = sale_obj.sudo().browse(self.sale_order_to_merge.ids)
        partners_list = []
        partners_list_write = []
        line_list = []
        cancel_list = []
        ...
        for sale in sales:
            origin_list.append(sale.display_name)
        if self.sale_order:
            origin_list.append(self.sale_order.display_name)
        ...
        if self.type == 'exist_1':
                    ...
                    for line in merge_ids:
                        if self.sale_order.state in ['done', 'sale', 'confirmed', 'cancel']:
                            ...
                            if sale.id != self.sale_order.id:
                                ...
                                line_obj.create(vals)
            final_view_id=self.sale_order.id
            msg_body = _("This sale order has been created from: <b>%s</b>") % (msg_origin)
            ...
            for orders in cancel_list:
                for s_order in self.env['sale.order'].sudo().browse(orders):
